...
Usage:
  cd EuroferMicrostructure/Expanded_Eurofer_CD/codes
...
def build_sim(solver_mode, physics_option=None):
    """Construct an ExpandedEuroferCDSimulation with the reference parameters."""
...
def _load_run_from_dir(d, solver_mode, label):
    ...
    from Expanded_Eurofer_CD.py_utils.post_process import calculate_derived_quantities
    saved = sys.stdout, sys.stderr
    sys.stdout = sys.stderr = io.StringIO()
    try:
...
def write_report(res_dict, wall_dict, report_path):
    ...
    lines += [sep,
              ...
              f'Expanded_Eurofer_CD  —  {PHYSICS_OPTION}  —  I=V=10,000',
              sep, '']
