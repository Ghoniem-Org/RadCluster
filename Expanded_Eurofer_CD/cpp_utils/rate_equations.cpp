...
static inline double K_ii_coal(const Parameters& P, int n, int np) {
    ...
    double size_factor = std::cbrt(static_cast<double>(n))
                       + std::cbrt(static_cast<double>(np));
    return P.Z_ii * P.A_sph_inv_O23 * size_factor * D_np;
}
...
#pragma omp parallel for if(use_omp) schedule(dynamic) num_threads(omp_threads)
    ...
    for (int m = 0; m <= x_hi_v_win; ++m) {
        ...
        for (int n = 1; n < std::min(3, P.i_mobile); ++n) {
            ...
            dcv[m] -= P.KVI[m] * cn * cm;
            if (m + n < V)
                dcv[m] += P.KVI[m + n] * cn * std::max(c_v[m + n], 0.0);
        }
    ...
    // Two channels: (a) mobile SIA hitting voids, (b) mobile vacancies hitting SIA.
    {
        ...
        for (int n = 0; n < I; ++n) {
            ...
            if (n == 0) {
                ...
                for (int m = 0; m < V; ++m)
                    mutual += sn * P.K_3D_cav_pref * P.m13[m] * cn * std::max(c_v[m], 0.0);
            } else if (n >= 3 && n < P.i_mobile) {
                for (int m = 0; m < V; ++m)
                    mutual += sn * K_1D_eff(P, n, m) * cn * std::max(c_v[m], 0.0);
            }
...
int rhs_bin_moment(sunrealtype t, N_Vector yv, N_Vector ydotv, void* user_data) {
    ...
    for (int m = 0; m < V; ++m) {
        ...
        for (int n = 1; n < std::min(3, P.i_mobile); ++n) {
            ...
            if (m + n < V)
                dc_v[m] += P.KVI[m + n] * cn * std::max(c_v[m + n], 0.0);
        }
    ...
    // J_SIA_mutual: ALL SIA content lost to SIA-vacancy annihilation (bin_moment).
    {
        ...
        for (int n = 0; n < I; ++n) {
            ...
            if (n == 0) {
                ...
                for (int m = 0; m < V; ++m)
                    mutual += sn * P.K_3D_cav_pref * P.m13[m] * cn_val * std::max(c_v[m], 0.0);
            } else if (n >= 3 && n < P.i_mobile) {
                for (int m = 0; m < V; ++m)
                    mutual += sn * K_1D_eff(P, n, m) * cn_val * std::max(c_v[m], 0.0);
            }
            // (b) Mobile vacancy hitting this SIA cluster
            for (int mp = 1; mp <= P.v_mobile; ++mp) {
                const double c_mp = std::max(c_v[mp - 1], 0.0);
