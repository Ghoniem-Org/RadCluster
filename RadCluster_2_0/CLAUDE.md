# RadCluster_2_0 — Physics and Solver Reference
...
`RadClusterSimulation` accepts either the new `(equations, cascade)` pair or the legacy single `physics_option=` kwarg. Legacy `equations='full_CD'` is silently aliased to `'discrete'`. Helpers `make_physics_option(eq, cas)` / `split_physics_option(po)` are exported from `RadCluster_2_0.py_utils`.
