...
Usage:
  cd EuroferMicrostructure/RadCluster_2_0/codes
...
def _load_run_from_dir(d, solver_mode, label):
    ...
    from RadCluster_2_0.py_utils.post_process import calculate_derived_quantities
    saved = sys.stdout, sys.stderr
    sys.stdout = sys.stderr = io.StringIO()
    try:
...
def write_report(res_dict, wall_dict, report_path):
    ...
    lines += [sep,
              ...
              f'RadCluster_2_0  —  {PHYSICS_OPTION}  —  I=V=10,000',
              sep, '']
