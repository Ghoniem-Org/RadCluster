/**
 * solver.cpp — RadCluster_2_0 main C++ ODE solver.
 *
 * Drives the RadCluster_2_0 cluster dynamics system for bcc Fe / EUROFER97.
...
int main(int argc, char* argv[]) {
    ...
    std::cerr << "RadCluster_2_0 solver: N_eq=" << N_EQ
    ...
    for (int i = 1; i < P.n_points; ++i) {
        ...
        if (retval >= 0) {
            ...
            if (is_bin) {
                ...
                // Bin edges are consumed from the explicit arrays transmitted
                // by Python (P.vac_bin_lo/hi) — NOT re-derived from r_ratio.
                if (Kv > 0) {
                    ...
                    if (PM >= 2) {
                        ...
                        for (int k = 0; k < Kv; ++k) {
                            int mlo = P.vac_bin_lo[k];
                            int mhi = P.vac_bin_hi[k];
