"""
input_data.py — InputData class for RadCluster_2_0.

Reads material, irradiation, and model parameters from a 5-sheet Excel
workbook (RadCluster_2_0/input/input_parameters.xlsx) and computes
...
class InputData:
    """
    Material, irradiation, and model parameters for RadCluster_2_0.
    ...
    def __init__(self, excel_file=INPUT_FILE, I=None, V=None,
        ...
        if not self.excel_file.is_file():
            raise FileNotFoundError(
                ...
                f"Run RadCluster_2_0/create_excel.py to generate it."
            )
    ...
    def _calculate_derived(self):
        ...
        # He production.
        # G_He_r (He production rate, appm He/dpa) is read from the Excel
        # Reactions sheet — this is the SINGLE AUTHORITATIVE SOURCE.  It is
        # stored in derived['G_He_r'] and forwarded to
        # defect_production.production_rates(); there is no hard-coded
        # duplicate in the FISSION/FUSION spectrum dicts (Stage-2 fix).
        # The spectrum-appropriate defaults below (0.75 fission / 10.0 fusion)
        # apply only when the Excel value is absent entirely.
        spectrum = str(re.get('spectrum', 'fission')).lower()
        _G_He_r_default = 0.75 if 'fiss' in spectrum else 10.0   # appm He/dpa
        G_He_r = float(re.get('G_He_r', _G_He_r_default))
