"""
rate_equations.py — ODE right-hand side for RadCluster_2_0.

REFERENCE ONLY — not used by the solver; the production RHS is the C++
implementation.  May be stale.  Scheduled for replacement by the Stage-3
graph-walker.  The `ode_system` / `_rhs_case1` / `_rhs_case2` methods below
are dead code: simulation.py integrates exclusively via the C++ solver and
never calls these Python RHS methods.  They have drifted from the C++
(Python RHS is mono-defect only) and are kept solely as a physics reference
until the Stage-3 graph-walker replaces them.
