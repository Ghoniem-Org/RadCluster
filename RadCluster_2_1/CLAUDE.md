# RadCluster_2_1 — Physics and Solver Reference
...
## 0. RadCluster_2_1 development objectives

`RadCluster_2_1` is a clone of `RadCluster_2_0` (the graph-based two-layer
cluster-dynamics framework) that adds two new physics capabilities. Both are
declared as new **populations / edges / kernels** in the EUROFER-97 host RAG
(`py_utils/materials/eurofer97/declaration.py`) and reuse the existing solver,
reductions, and conservation machinery — no abstract-core or integrator change.
...
- **Network-only loss** `Λ_n^net = v_net·ρ_net·w_c·P_ℓd(n)` — a sweep frequency
  that stays nonzero for sessile loops (the population that dominates
  incorporation); deliberately *not* tied to the loop's own diffusivity, and
  *not* duplicating the existing SIA–SIA coalescence / Marian junction.
- **`ρ_net` evolves by operator splitting** — held piecewise-constant within
  each integration segment and refreshed between segments via `rebuild_rates()`.
  This keeps the banded + low-rank Jacobian structure intact, so the **Woodbury
  preconditioner needs no rework**.
- **Conservation:** the `n` SIAs per absorbed loop are routed into the existing
  `J_SIA_fixed` cumulative-sink integral, so `δ_FP` (§8) is preserved unchanged.
- Gated behind a `LOOP_NETWORK_LOSS` flag (default off → legacy 2_0 behaviour).

### (b) RIS / precipitation — key design commitments (see plan)
- RIS enters as a **two-reservoir (bulk + sink-local) 0-D reduction**;
  enrichment/depletion sign and `T`-dependence come from **offline-tabulated
  transport coefficients** `d_sV(T), d_sI(T)`, not free fit parameters.
- EUROFER-97 calibration **leads with Cr-rich α′** (Ni/Si are deliberately
  minimised, so MNS is weak — kept as a secondary, T91-validated channel).
- Needs a **general `SOLUTE_TRAPPING` walker handler** (the He reduction is
  special-cased today) and a per-solute conservation diagnostic `δ_s` (analogue
  of `δ_FP`/`δ_He`, target `< 1e-6`).
- **C++ kernel mirroring is the dominant cost** — physics is developed and
  validated through the Python graph walker first, then ported to
  `cpp_utils/materials/eurofer97/rate_kernels.cpp`.

Both features follow the same phased discipline: **Python walker → conservation
gate → C++ mirror → calibrate**. See each plan document for the full phase list.
...
`RadClusterSimulation` accepts either the new `(equations, cascade)` pair or the legacy single `physics_option=` kwarg. Legacy `equations='full_CD'` is silently aliased to `'discrete'`. Helpers `make_physics_option(eq, cas)` / `split_physics_option(po)` are exported from `RadCluster_2_1.py_utils`.
