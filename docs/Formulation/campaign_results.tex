...
\newcommand{\dnr}{\textit{did not reach}}
...
solution for the closure verification. Two defects found \emph{by} the campaign
--- a physics channel missing from one solver path, and a dose ladder that
scored the wrong dose --- are documented alongside the numbers they invalidated,
...
Table 4 (C0--C4) & bin\_moment & 4\,000 & 4\,000 & 1600--5 & 2--18 & 2 & 2.000 dpa \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
Rung & $N_{\rm eq}$ & $N_{111}$ (\SI{e21}{}) & $N_{100}$ (\SI{e21}{}) & $d_{100}$ (nm) & $N_{\rm cav}$ (\SI{e20}{}) & $d_{\rm cav}$ (nm) \\
\midrule
\textbf{D (exact)} & \textbf{8006} & \textbf{12.568} & \textbf{1.065} & \textbf{5.741} & \textbf{2.948} & \textbf{2.544} \\
\midrule
...
C1 ($1/10$)  & 830 & 12.335 ($-1.9\%$) & 1.044 ($-2.0\%$) & 5.690 ($-0.9\%$) & 2.555 ($-13.3\%$) & 2.273 ($-10.6\%$) \\
C2 ($1/40$)  & 244 & 12.144 ($-3.4\%$) & 1.044 ($-2.0\%$) & 5.681 ($-1.0\%$) & 2.574 ($-12.7\%$) & 2.279 ($-10.4\%$) \\
C3 ($1/160$) & 108 & 7.027 ($-44.1\%$) & 1.018 ($-4.4\%$) & 5.349 ($-6.8\%$) & 3.219 ($+9.2\%$) & 2.451 ($-3.6\%$) \\
C4 ($1/800$) & 84  & 6.142 ($-51.1\%$) & 0.953 ($-10.5\%$) & 5.678 ($-1.1\%$) & 6.575 ($+123.0\%$) & 2.660 ($+4.6\%$) \\
\midrule
C4, $P=1$    & 51  & 17.498 ($+39.2\%$) & 0.059 ($-94.5\%$) & 10.755 ($+87.3\%$) & 20.929 ($+610\%$) & 2.632 ($+3.5\%$) \\
\bottomrule
\end{tabular}}
\end{table}
...
\textbf{The bands do not overlap the curves, and that is the point.} These runs
stop at \SI{2}{dpa}; the loop measurements begin at \SI{14.6}{dpa}. Table~4 is
a verification exercise at low dose, and nothing in these figures should be
read as agreement or disagreement with experiment. The band is drawn where the
data actually are so that the unconstrained region cannot be mistaken for a
validated one.
