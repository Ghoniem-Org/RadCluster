...
\end{equation}
which we parametrise by retaining the physics that only the $\langle100\rangle$
\emph{prelogarithmic} part softens, with the exponent fixed at $1/4$ by the
analytic scaling $\hat F_{001}([100])\propto\sqrt{c_{11}-c_{12}}\propto
(1-T/T_c)^{1/4}$ of Eq.~\eqref{eq:F100}:
\begin{equation}
  \boxed{\;f_{100}(T) = f_{100}^{\rm core} + f_{100}^{\rm pre}\,(1-T/T_c)^{1/4},
  \qquad \Delta f(T) = f_{111} - f_{100}(T)\;}
  \label{eq:delta-f}
\end{equation}
where $f_{100}^{\rm core}=(F_\delta+F_c)_{100}$ is the temperature-independent
core energy and $f_{100}^{\rm pre}=\hat F_{001}\ln(4R^\ast/e\delta)$ the softening
prelogarithmic part (constants from Table~\ref{tab:core-constants}).  The single
fit quantity is the crossover temperature $T^\ast$ (where $\Delta f=0$), chosen
so that conversion turns on near $350\,^\circ$C and is strongly favoured by
$550\,^\circ$C, reproducing the three stability regions of \citet{Dudarev2008}.
Conversion is thermodynamically admissible for $T>T^\ast$.
...
softening exp.   & exponent in $\Delta f(T)$ & $1/4$ (fixed) & \citet{Dudarev2008}, Eq.~\eqref{eq:F100} \\
$T^\ast$         & conversion crossover temperature & cal.\ ($\in[350,550]\,^\circ$C) & \citet{Dudarev2008} \\
...
$A_{100},B_{100}$& $\langle100\rangle$ binding-energy fit & $0.7160,\ 0.3581$ & Table~18 (DFT) \\
